...
open import Data.Nat.Properties using (+-comm; *-comm; +-identityʳ; *-identityʳ)
```
...
```agda
mytrans : ∀ {m n p : ℕ} → m ≤ n → n ≤ p → m ≤ p
mytrans z≤n _ = z≤n
mytrans (s≤s mn) (s≤s np) = s≤s (mytrans mn np)
```
## Anti-symmetry
...
```agda
my<-trans : ∀ {i j k : ℕ} → i < j → j < k → i < k
my<-trans z<s (s<s _) = z<s
my<-trans (s<s ij) (s<s jk) = s<s (my<-trans ij jk)
```
...
<-tri : ∀ (m n : ℕ) → Tri m n
<-tri zero zero = eq refl
<-tri zero (suc n) = lt z<s
<-tri (suc m) zero = gt (flip z<s)
<-tri (suc m) (suc n) with <-tri m n
...                      | lt mltn = lt (s<s mltn)
...                      | eq meqn = eq (cong suc meqn)
...                      | gt (flip mltn) = gt (flip (s<s mltn)) 
```
...
--mono zero (suc n) zero q mn pq = z<s
--mono zero (suc n) (suc p) (suc q) mn (s<s pq) = s<s (my<-trans pq {!!})
--mono (suc m) n p q mn pq = {!!}
```
...
```agda
lem1 : ∀ {m n : ℕ} → suc m ≤ n → m < n
lem1 (s≤s z≤n) = z<s
lem1 (s≤s (s≤s mn)) = s<s (lem1 (s≤s mn))

lem2 : ∀ {m n : ℕ} → m < n → suc m ≤ n
lem2 z<s = s≤s z≤n
lem2 (s<s mn) = s≤s (lem2 mn)
```
...
lem3 : ∀ (m : ℕ) → m ≤ suc m
lem3 0 = z≤n
lem3 (suc m) = s≤s (lem3 m)

trans2 : ∀ (m n p : ℕ) → m < n → n < p → m < p
trans2 m n p mn np = lem1 (mytrans (mytrans (lem2 mn) (lem3 n)) (lem2 np))
```
...
```agda
addsuc : ∀ (m n : ℕ) → m + suc n ≡ suc (m + n)
addsuc zero n = refl
addsuc (suc m) n = cong suc (addsuc m n)
--e+o≡o : ∀ {m n : ℕ} → even m → odd n → odd (m + n)
--e+o≡o {m} {suc n} em (suc en) rewrite addsuc m n = suc (e+e≡e em en)
o+o≡e : ∀ {m n : ℕ} → odd m → odd n → even (m + n)
o+o≡e {suc m} {suc n} (suc em) (suc en) rewrite addsuc m n = suc (suc (e+e≡e em en))
```
...
data One : Bin → Set where
  base : One (⟨⟩ I)
  add1 : ∀ {b : Bin} → One b → One (b I)
  add0 : ∀ {b : Bin} → One b → One (b O)
...
incone base = add0 base
incone (add1 oneb) = add0 (incone oneb)
incone (add0 oneb) = add1 oneb
...
{-
posofone : ∀ {b : Bin} → One b → 1 ≤ from b
posofone base = s≤s z≤n
posofone (add1 oneb) = {!!}
posofone (add0 oneb) = {!!}
convcan : ∀ {b : Bin} → Can b → to (from b) ≡ b
convcan zero = refl
convcan (one x) = {!!}
-}
```
