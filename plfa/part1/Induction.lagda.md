...
mycomm : ∀ (m n : ℕ) → m + n ≡ n + m
mycomm zero n = begin
  zero + n
  ≡⟨⟩ n
  ≡⟨ sym (+-identityʳ n) ⟩ n + zero
  ∎
mycomm (suc m) n = begin
  (suc m) + n
  ≡⟨⟩ suc (m + n)
  ≡⟨ cong suc (mycomm m n) ⟩ suc (n + m)
  ≡⟨ sym (+-suc n m) ⟩ n + suc m
  ∎
```
...
+-assoc-3 n p = begin
  ...
  ≡⟨⟩ suc (2 + n) + p
  ≡⟨⟩ suc (2 + n + p)
  ≡⟨ cong suc (+-assoc-2 n p) ⟩ suc (2 + (n + p))
  ≡⟨⟩ 3 + (n + p)
  ∎
```
...
```agda
*-zeror : ∀ (m : ℕ) → m * zero ≡ zero
*-zeror zero = refl
*-zeror (suc m) rewrite *-zeror m = refl
...
*-comm : ∀ (m n : ℕ) → m * n ≡ n * m
*-comm zero n rewrite *-zeror n = refl
*-comm (suc m) n rewrite *-sucr n m | *-comm m n = refl
```
...
```
^-distrib1 : ∀ (m n p : ℕ) → m ^ (n + p) ≡ (m ^ n) * (m ^ p)
^-distrib1 m zero p rewrite +-identityʳ (m ^ p) = refl
^-distrib1 m (suc n) p
  rewrite ^-distrib1 m n p
  | *-assoc m (m ^ n) (m ^ p) = refl

^-distrib2 : ∀ (m n p : ℕ) → (m * n) ^ p ≡ (m ^ p) * (n ^ p)
^-distrib2 m n zero = refl
^-distrib2 m n (suc p)
  rewrite ^-distrib2 m n p
  | *-assoc m n ((m ^ p) * (n ^ p))
  | sym (*-assoc n (m ^ p) (n ^ p))
  | *-comm n (m ^ p)
  | *-assoc (m ^ p) n (n ^ p)
...
^-assoc : ∀ (m n p : ℕ) → (m ^ n) ^ p ≡ m ^ (n * p)
^-assoc m n zero rewrite *-zeror n = refl
^-assoc m n (suc p)
  rewrite ^-assoc m n p
  | *-sucr n p
  | ^-distrib1 m n (n * p)
...
lemma1 : ∀ (m : ℕ) → m + 1 ≡ suc m
lemma1 m rewrite +-suc′ m 0 | +-identityʳ m = refl
law1 : ∀ (b : Bin) → from (inc b) ≡ suc (from b)
law1 ⟨⟩ = refl
law1 (b O) rewrite +-identityʳ (from b) | lemma1 ((from b) + (from b)) = refl
law1 (b I)
  rewrite law1 b
  | +-identityʳ (from b)
  | +-suc′ (from b) (from b)
  | lemma1 ((from b) + (from b))
  = refl

law3 : ∀ (m : ℕ) → from (to m) ≡ m
law3 zero = refl
law3 (suc m) rewrite law1 (to m) | law3 m = refl
