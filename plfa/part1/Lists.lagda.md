...
```agda
revrev : ∀ {A : Set} (xs ys : List A) → reverse (xs ++ ys) ≡ reverse ys ++ reverse xs
revrev [] ys rewrite ++-identityʳ (reverse ys) = refl
revrev xs [] rewrite ++-identityʳ xs = refl
revrev (x ∷ xs) (y ∷ ys) rewrite revrev xs (y ∷ ys)
...
```agda
revinv : ∀ {A : Set} (xs : List A) → reverse (reverse xs) ≡ xs
revinv [] = refl
revinv (x ∷ xs) rewrite revrev (reverse xs) [ x ] | revinv xs = refl
```
...
mapcomp :  ∀ {A B C : Set} (f : A → B) (g : B → C) → map (g ∘ f) ≡ map g ∘ map f
mapcomp {A} f g = extensionality λ l → helper l where
  helper : ∀ (l : List A) → map (g ∘ f) l ≡ (map g ∘ map f) l
  helper [] = refl
  helper (x ∷ xs) rewrite helper xs = refl
```
...
map-Tree f g (node t1 x t2) = node (map-Tree f g t1) (g x) (map-Tree f g t2)
```
...
As another example, observe that
    ...
    foldr _∷_ [] xs ≡ xs
```agda
lem1 : ∀ {A : Set} (xs : List A) → foldr _∷_ [] xs ≡ xs
lem1 [] = refl
lem1 (x ∷ xs) rewrite lem1 xs = refl
```
...
```agda
lem2 : ∀ {A : Set} (xs ys : List A) → xs ++ ys ≡ foldr _∷_ ys xs
lem2 [] ys = refl
lem2 (x ∷ xs) ys rewrite lem2 xs ys = refl
```
...
mapisfold {A} f = extensionality λ l → helper l where
  helper : ∀ (l : List A) → (map f) l ≡ (foldr (λ x xs → f x ∷ xs) []) l
  helper [] = refl
  helper (x ∷ xs) rewrite helper xs = refl
```
...
fold-Tree f g (node t1 y t2) = g (fold-Tree f g t1) y (fold-Tree f g t2)
```
